...
def visible_lines(text: str, *, markdown: bool = False):
    """Yield numbered lines outside CommonMark-style fenced code blocks.

    JSONL callers must leave Markdown handling disabled.
    """
    fence_char = ""
    fence_length = 0
...
def find_conflict_markers(text: str, *, markdown: bool = False) -> list[int]:
    """Return 1-based marker line numbers, excluding fences only for Markdown."""
    return [lineno for lineno, line in visible_lines(text, markdown=markdown)
...
def find_duplicate_h1(text: str) -> list[int]:
    ...
    h1_lines = [lineno for lineno, line in visible_lines(text, markdown=True)
                if line.startswith("# ")]
    return h1_lines[1:]
...
def integrity_errors(
    ...
    *,
    ...
    (§P3 append-at-tail invariants)."""
    errors: list[str] = []
    for lineno in find_conflict_markers(text, markdown=path.suffix.lower() == ".md"):
        errors.append(
