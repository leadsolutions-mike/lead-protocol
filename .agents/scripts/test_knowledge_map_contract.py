...
def test_seed_is_generic_pointer_only_with_real_targets():
    ...
    for topic, target, locator in rows:
        ...
        assert locator.strip().strip('`') in (ROOT / target).read_text(encoding="utf-8"), locator
...
def test_discovery_is_on_demand_with_independent_fallback():
    ...
    kernel = (ROOT / '.agents/PROTOCOL_RULES.md').read_text(encoding="utf-8")
...
def test_maintenance_is_in_quality_and_close_without_schema_growth():
    kernel = (ROOT / '.agents/PROTOCOL_RULES.md').read_text(encoding="utf-8")
    for section in [kernel.split('### Session close ritual', 1)[1].split('### Branch ordering', 1)[0], kernel.split('## §P4', 1)[1].split('## §P5', 1)[0], (ROOT / '.agents/PROJECT_RULES.md').read_text(encoding="utf-8").split('## §J5', 1)[1].split('## §J6', 1)[0]]:
